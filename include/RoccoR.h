...
class Rochester_Corrector {

    public:
        
        /*
        Rochester_Corrector();
        Rochester_Corrector(const std::string& file_resolution) {

            fRochester_corrector.init(file_resolution);

            fRandom.SetSeed(std::time(nullptr));

        }
        */

        virtual ~Rochester_Corrector(){};

        void Initialize(const std::string& file_resolution) {
        ...
        double GetCorrection( const int& charge, const double& pt, const double& eta, const double& phi, const bool& isMatched, const double& gen_pt, const int& n_layers, const bool& isData ) {

            if ( fRochester_corrector.empty() ) {
            ...
            if ( isData ) scale_factor = fRochester_corrector.kScaleDT(charge, pt, eta, phi, 0, 0);

            else {

                if ( isMatched ) scale_factor = fRochester_corrector.kSpreadMC(charge, pt, eta, phi, gen_pt, 0, 0);

                else scale_factor =  fRochester_corrector.kSmearMC(charge, pt, eta, phi, n_layers, gRandom->Rndm(), 0, 0);
