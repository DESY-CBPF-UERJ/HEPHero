    ...
    * `calculate_morphing_weight_gradient()` calculates the gradient of the morphing weights, `grad_theta w_b(theta)`.
    ...
    def calculate_morphing_matrix(self, basis=None, gp=None, gd=None, gs=None):
        ...
        if self.gd is None and self.gs is None and self.gp is None:
            ...
            #print("n_benchmarks", n_benchmarks)
    ...
    def get_component_weights(self, c_idx, theta):
        factor = np.ones(len(theta))
        for p in range(self.n_parameters):
            factor = factor*(theta[:,p]**self.components[c_idx, p])
    ...
    def calculate_morphing_weights(
        ...
        ----------
        theta : ndarray
            Parameter point `theta` with shape `(n_events, n_parameters)`.
        ...
        # calculate matrix with previous method
        if basis is None:
            basis = self.basis
        ...
        c_idx = range(self.n_components)
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.cpu_count) as executor:
            component_weights = list(executor.map(self.get_component_weights, c_idx, repeat(theta)))
        component_weights = np.array(component_weights).T

        morphing_weights = morphing_matrix.T.dot(component_weights.T).T

        # Transform to basis weights
        # Shape (n_events, n_benchmarks_phys)
        return morphing_weights


    def get_component_weight_gradient(self, c_idx, theta):
        component_weight_gradients_c = np.zeros((len(theta), self.n_parameters))
        for i in range(self.n_parameters):
            factor = np.ones(len(theta))
            for p in range(self.n_parameters):
                if p == i and self.components[c_idx, p] > 0:
                    factor = factor * self.components[c_idx, p] * theta[:,p]**(self.components[c_idx, p] - 1)
                elif p == i:
                    factor = np.zeros(len(theta))
                    break
                else:
                    factor = factor * theta[:,p]**self.components[c_idx, p]
            component_weight_gradients_c[:, i] = factor

        return component_weight_gradients_c

    def calculate_morphing_weight_gradient(self, theta, basis=None, morphing_matrix=None, gp=None, gd=None, gs=None, device=None):
        """
        ...
        # Transform to basis weights
        # Shape (n_events, n_parameters, n_benchmarks_phys)
        return morphing_weight_gradient
