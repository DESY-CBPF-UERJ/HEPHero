...
stat_values = tools.features_stat(model_type, ds_full_train, ds_full_test, vec_full_train, vec_full_test, variables, vec_variables, var_names, vec_var_names, var_use, vec_var_use, class_names, class_labels, class_colors, plots_outpath, parameters=model[N])
...
class_model, iteration, train_acc, test_acc, train_loss, test_loss, domains_train_acc, domains_test_acc, domains_train_loss, domains_test_loss, feature_score_info = tools.train_model(
    input_path,
    N_signal,
    train_frac,
    load_size_training,
...
for idom in range(len(domains)):
    ...
    ax2 = plt.subplot(gs1[0])
    plt.axvline(position, color='grey')
    plt.plot(iteration, domains_train_loss[:,idom], "-", color='red', label='Train (Domain Loss)')
    plt.plot(iteration, domains_test_loss[:,idom], "-", color='blue', label='Test (Domain Loss)')
    #plt.yscale('log')
    ax2.set_xlabel("iterations", size=14, horizontalalignment='right', x=1.0)
    ax2.set_ylabel("Loss", size=14, horizontalalignment='right', y=1.0)
    ax2.tick_params(which='major', length=8)
...
for i in range(n_outputs):
    ...
    for ikey in range(len(class_names)):
        yH, errH = tools.step_plot( ax1, var, ds_full_train[ds_full_train["class"] == ikey], label=class_labels[ikey]+" (train)", color=class_colors[ikey], weight="mvaWeight", bins=bins, error=True, linestyle='dotted' )
        yTrain.append(yH)
        errTrain.append(errH)
        yH, errH = tools.step_plot( ax1, var, ds_full_test[ds_full_test["class"] == ikey], label=class_labels[ikey]+" (test)", color=class_colors[ikey], weight="mvaWeight", bins=bins, error=True )
        yTest.append(yH)
        errTest.append(errH)
    ...
    if model[N][4] == "cce" or model[N][4] == "bce":
        ...
        ctr_train.roc_plot(label='ROC (train)', color='blue', linestyle="--", version=1)
        ctr_test = tools.control( var, signal_test_roc, bkg_test_roc, weight="evtWeight", bins=np.linspace(0,1,1001) )
        ctr_test.roc_plot(label='ROC (test)', color='blue', linestyle="-", version=1)
        plt.text(0.05, 0.1, r'$\mathrm{AUC}\ (\mathrm{test})$ = '+str(format(ctr_test.auc(), '.3f')))
    
        ax1.set_xlabel(r"Background rejection $(1-\epsilon_b)$", size=14, horizontalalignment='right', x=1.0)
        ax1.set_ylabel(r"Signal efficiency $(\epsilon_s)$", size=14, horizontalalignment='right', y=1.0)
        #plt.xscale('log')
        ax1.set_ylim([0,1])
        ax1.set_xlim([0,1])
...
if model[N][4] == "cce":
    ...
    for i in range(n_outputs):
        pred_name = 'score_C'+str(i)
        full_test_predictions.append(ds_full_test[pred_name])
    full_test_predictions = np.array(full_test_predictions)
    full_test_predictions = np.transpose(full_test_predictions, axes=[1, 0])
    full_test_predictions = np.argmax(full_test_predictions, axis=1)
    ds_full_test["class_pred"] = full_test_predictions
    ...
    ax1 = plt.subplot(gs1[0])
    #==================================================
    tools.confusion_matrix_plot(ax1, ds_full_test["class"], ds_full_test["class_pred"], ds_full_test["evtWeight"], class_names)
    
    plt.subplots_adjust(left=0.25, bottom=0.20, right=0.97, top=0.95, wspace=0.18, hspace=0.165)
    plt.savefig(os.path.join(model_outpath, "confusion_matrix.png"), dpi=400)
    plt.savefig(os.path.join(model_outpath, "confusion_matrix.pdf"))
