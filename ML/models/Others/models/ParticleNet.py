...
class ParticleNet(nn.Module):
    ...
    def __init__(self,
        ...
        elif parameters[4] == 'bce' and num_classes == 2:
        ...
        if self.for_segmentation:
            #fcs.append(nn.Conv1d(fc_params[-1][0], num_classes, kernel_size=1)) #original
            fcs.append(nn.Conv1d(fc_params[-1][0], n_output, kernel_size=1))
        else:
            #fcs.append(nn.Linear(fc_params[-1][0], num_classes)) #original
            fcs.append(nn.Linear(fc_params[-1][0], n_output))
...
#         assert(((fts.abs().sum(dim=1, keepdim=True) != 0).float() - mask.float()).abs().sum().item() == 0)
        ...
        #    output = torch.softmax(output, dim=1) #original
...
class ParticleNetTagger(nn.Module):
    ...
    def __init__(self,
        ...
        #-------------------------------------------------------------------------------------
        stat_mean = stat_values["mean"]
        stat_std = stat_values["std"]
        stat_dim = stat_values["dim"]

        # use vec_types in the future
        self.pf_mean = torch.FloatTensor(np.ones(stat_dim[0]))
        self.pf_std = torch.FloatTensor(np.ones(stat_dim[0]))
        self.sv_mean = torch.FloatTensor(np.ones(stat_dim[1]))
        self.sv_std = torch.FloatTensor(np.ones(stat_dim[1]))
        for i in range(len(stat_mean[0])):
            self.pf_mean[i] = self.pf_mean[i]*stat_mean[0][i]
            self.pf_std[i] = self.pf_std[i]*stat_std[0][i]
        for i in range(len(stat_mean[1])):
            self.sv_mean[i] = self.sv_mean[i]*stat_mean[1][i]
            self.sv_std[i] = self.sv_std[i]*stat_std[1][i]

        if device == "cuda":
            self.pf_mean = self.pf_mean.to("cuda")
            self.pf_std = self.pf_std.to("cuda")
            self.sv_mean = self.sv_mean.to("cuda")
            self.sv_std = self.sv_std.to("cuda")
        #-------------------------------------------------------------------------------------
    ...
    def stat_device(self, dev):
        self.pf_mean = self.pf_mean.to(dev)
        self.pf_std = self.pf_std.to(dev)
        self.sv_mean = self.sv_mean.to(dev)
        self.sv_std = self.sv_std.to(dev)
    ...
    def forward(self, pf_points, pf_features, pf_mask, sv_points, sv_features, sv_mask):
        ...
        #-------------------------------------------------------------------------------------
        pf_mean = self.pf_mean*pf_mask
        sv_mean = self.sv_mean*sv_mask

        pf_features = (pf_features - pf_mean)/self.pf_std
        sv_features = (sv_features - sv_mean)/self.sv_std
        #-------------------------------------------------------------------------------------
